...
static netif *enumInterfaces(JNIEnv *env) {
  ...
  int sock6 = openSocket(env, AF_INET6);
  if (sock6 < 0 && (*env)->ExceptionOccurred(env)) {
    return NULL;
  }
  ...
  for (ifa = origifa; ifa != NULL; ifa = ifa->ifa_next) {
    if (ifa->ifa_addr != NULL) {
      switch (ifa->ifa_addr->sa_family) {
        case AF_PACKET:
          ifs = addif(env, 0, ifa, ifs);
          break;
        case AF_INET:
          ifs = addif(env, sock, ifa, ifs);
          break;
        case AF_INET6:
          ifs = addif(env, sock6, ifa, ifs);
          break;
      }
    }
  }
...
netif *addif(JNIEnv *env, int sock, struct ifaddrs *ifa, netif *ifs)
{
   ...
   * logical interfaces.
   */
  strncpy(name, ifa->ifa_name, ifnam_size);
  name[ifnam_size - 1] = '\0';
  *vname = 0;
