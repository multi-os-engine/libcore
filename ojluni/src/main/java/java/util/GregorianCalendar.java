...
/////////////////////////////
    ...
    /**
     * Adjust the given time in millis so that it takes into account the Zone Offset and the
     * Daylight Savings Time (DST) rules, if any.
     *
     * <p>The {@code standardTimeInGMT} value supplied was calculated as if the fields represented
     * a standard (no DST) time in the {@code GMT} time zone.
     *
     * <p>The standard time in the supplied zone is calculated by subtracting the zone offset from
     * the standard time in {@code GMT}. So, for example Los Angeles is 8 hours behind {@code GMT}
     * so its zone offset is -28800000. When subtracted from {@code standardTimeInGMT} that
     * actually adds 8 hours to {@code standardTimeInGMT} which is what is expected as we know that
     * {@code 14:00} in Los Angeles is after {@code 14:00} in {@code GMT}.
     *
     * <p>The standard time in the zone is then used to retrieve the zone and DST offsets from the
     * time zone. See {@link #adjustDstOffsetForInvalidWallClock(long, TimeZone, int)} for more
     * information.
     *
     ...
     * @param standardTimeInGMT the time in millis, calculated assuming the time zone was GMT.
    ...
    private long adjustForZoneAndDaylightSavingsTime(
            int fieldMask, int tzMask, long standardTimeInGMT, TimeZone zone) {
        ...
        if (tzMask != (ZONE_OFFSET_MASK|DST_OFFSET_MASK)) {
            ...
            // Calculate the standard time (no DST) in the supplied zone.
            long standardTimeInZone = standardTimeInGMT - gmtOffset;
        ...
        long standardTimeInZone = standardTimeInGMT - zoneOffset;
