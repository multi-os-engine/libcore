...
public final class Currency implements Serializable {
    ...
    private static Currency getInstance(android.icu.util.Currency icuCurrency) {
        synchronized (instances) {
            Currency currency = instances.get(icuCurrency.getCurrencyCode());
            if (currency == null) {
        ...
        synchronized (available) {
            if (available == null) {
                ...
                for (android.icu.util.Currency icuCurrency : icuAvailableCurrencies) {
                    available.add(getInstance(icuCurrency));
                }
            }
