 ...
 */
public final class TimeZoneNamesProfiler {
    ...
    public static void main(String[] args) throws Exception {
      dalvik.system.VMDebug.startMethodTracing("/data/local/tmp/tznames.trace", 200*1024*1024, 1, true, 50); 
...
//      cachedZoneStrings.get(Locale.US);
    ...
    private static String[][] fillZoneStrings(long now, Locale locale) {
        ...
        for (int i = 0; i < result.length; i++) {
            ...
            String canonicalZoneId = android.icu.util.TimeZone.getCanonicalID(zoneId);

            row[OLSON_NAME] = zoneId;
            String longStd = timeZoneNames.getDisplayName(canonicalZoneId,
                android.icu.text.TimeZoneNames.NameType.LONG_STANDARD, now);
            String shortStd = timeZoneNames.getDisplayName(canonicalZoneId,
                android.icu.text.TimeZoneNames.NameType.SHORT_STANDARD, now);
            String longDst = timeZoneNames.getDisplayName(canonicalZoneId,
                android.icu.text.TimeZoneNames.NameType.LONG_DAYLIGHT, now);
            String shortDst = timeZoneNames.getDisplayName(canonicalZoneId,
                android.icu.text.TimeZoneNames.NameType.SHORT_DAYLIGHT, now);
