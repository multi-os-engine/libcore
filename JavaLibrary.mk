...
# Create the core-crypto library
include $(CLEAR_VARS)
...
LOCAL_MODULE_TAGS := optional
LOCAL_MODULE := core-crypto
LOCAL_ADDITIONAL_DEPENDENCIES := $(LOCAL_PATH)/JavaLibrary.mk

include $(BUILD_JAVA_LIBRARY)

# Create the core-crypto library without jarjar for tests
include $(CLEAR_VARS)
...
LOCAL_MODULE_TAGS := optional
LOCAL_MODULE := core-crypto-nojarjar
LOCAL_ADDITIONAL_DEPENDENCIES := $(LOCAL_PATH)/JavaLibrary.mk
...
LOCAL_NO_STANDARD_LIBRARIES := true
LOCAL_JAVA_LIBRARIES := bouncycastle core core-crypto-nojarjar core-junit okhttp
...
ifeq ($(WITH_HOST_DALVIK),true)
    ...
    # Make the core-crypto-hostdex library
    include $(CLEAR_VARS)
    LOCAL_SRC_FILES := $(call all-main-java-files-under,crypto)
    ...
    LOCAL_BUILD_HOST_DEX := true
    LOCAL_MODULE_TAGS := optional
    LOCAL_MODULE := core-crypto-hostdex
    LOCAL_ADDITIONAL_DEPENDENCIES := $(LOCAL_PATH)/JavaLibrary.mk
    include $(BUILD_HOST_JAVA_LIBRARY)

    # Make the core-crypto-hostdex-nojarjar for tests
    include $(CLEAR_VARS)
    LOCAL_SRC_FILES := $(call all-main-java-files-under,crypto)
    LOCAL_JAVA_LIBRARIES := core-hostdex
    LOCAL_NO_STANDARD_LIBRARIES := true
    ...
    LOCAL_BUILD_HOST_DEX := true
    LOCAL_MODULE_TAGS := optional
    LOCAL_MODULE := core-crypto-hostdex-nojarjar
    LOCAL_ADDITIONAL_DEPENDENCIES := $(LOCAL_PATH)/JavaLibrary.mk
    include $(BUILD_HOST_JAVA_LIBRARY)
    ...
    LOCAL_JAVA_RESOURCE_DIRS := $(test_resource_dirs)
    LOCAL_NO_STANDARD_LIBRARIES := true
    LOCAL_JAVA_LIBRARIES := bouncycastle-hostdex core-hostdex core-crypto-hostdex-nojarjar core-junit-hostdex okhttp-hostdex
