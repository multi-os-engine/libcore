...
public class StrictMathBenchmark extends SimpleBenchmark {
    ...
    private final double[] ceilDoubles = new double[] {
            3245817.2018463886,
            1418139.083668501,
    ...
    private final double[] floorDoubles = new double[] {
            7.140274745333553E11,
            3742491.296880909,
            -4.696725715628246E10,
            -1.9610339084804148E15,
    ...
    /* Note this benchmark runs StrictMath.ceil for every interesting value defined
       above: for average runtime of a single ceil call, divide the result by 
       ceilDoubles.length */
    public void timeCeil(int reps) {
        for (int rep = 0; rep < reps; ++rep) {
            for (int i = 0; i < ceilDoubles.length; ++i) {
                StrictMath.ceil(ceilDoubles[i]);
            }
        }
    }
    ...
    /* Note this benchmark runs StrictMath.floor for every interesting value defined
       above: for average runtime of a single floor call, divide the result by 
       floorDoubles.length */
    public void timeFloor(int reps) {
        for (int rep = 0; rep < reps; ++rep) {
            for (int i = 0; i < floorDoubles.length; ++i) {
                StrictMath.floor(floorDoubles[i]);
            }
        }
    }
