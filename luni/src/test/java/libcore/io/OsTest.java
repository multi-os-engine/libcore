...
public class OsTest extends TestCase {
  ...
  public void test_PacketSocketAddress() throws Exception {
    ...
    assertEquals((short) ETH_P_IPV6, bound.getProtocol());
    assertEquals(lo.getIndex(), bound.getInterfaceIndex());
    assertEquals(ARPHRD_LOOPBACK, bound.getHardwareType());
    assertEquals(0, bound.getPacketType());
    ...
    byte[] boundAddr = bound.getAddress();
