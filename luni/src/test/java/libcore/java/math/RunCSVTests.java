...
public class RunCSVTests extends CSVTest {
    ...
    void runTest(String func, double expectedOutput, double input, String extra)
        ...
        try {
            assertEquals(extra + ": " + m + ": " + input + ": ", expectedOutput, (double) returnValue, allowedError);
        } catch (ClassCastException e) {
            // returnValue is not a double, must be getExponent which returns int
            assertEquals(extra + ": " + m + ": " + input + ": ", (int) expectedOutput, (int) returnValue, allowedError);
        }
    }
    ...
    void run2InputTest(String func, double expectedOutput, double input1,
        ...
        try {
            assertEquals(extra + ": " + m + ": ", expectedOutput, (double) returnValue, allowedError);
        } catch (ClassCastException e) {
            // returnValue is not a double, must be getExponent which returns int
            assertEquals(extra + ": " + m + ": ", (int) expectedOutput, (int) returnValue, allowedError);
        }
    }
