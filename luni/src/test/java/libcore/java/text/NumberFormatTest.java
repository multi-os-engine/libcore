...
public class NumberFormatTest extends junit.framework.TestCase {
    ...
    // Test the pattern for formatting a currency is correctly used from ICU data.
    public void test_setCurrency() throws Exception {
        NumberFormat nf = NumberFormat.getCurrencyInstance(Locale.US);

        // Armenian Dram ISO 4217 code + 0 fractional digits.
        nf.setCurrency(Currency.getInstance("AMD"));
        assertEquals("AMD50", nf.format(50.00));

        // Euro sign + 2 decimal places.
        nf.setCurrency(Currency.getInstance("EUR"));
        assertEquals("€50.00", nf.format(50.00));

        // Japanese Yen symbol +  0 fractional digits.
        nf.setCurrency(Currency.getInstance("JPY"));
        assertEquals("¥50", nf.format(50.00));

        // Swiss Franc ISO 4217 code + 2 fractional digits.
