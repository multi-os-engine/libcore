...
public class MembershipKeyTest extends TestCase {
    ...
    // These are effectively final for the test
    static {
        try {
            MULTICAST_ADDRESS = InetAddress.getByName("224.0.0.2");
        } catch (UnknownHostException unHandledException) {}

        try {
            NETWORK_INTERFACE = NetworkInterface.getByName("lo");
        } catch (SocketException unHandledException) {}
    }
    ...
    private void test_drop() throws IOException {
        key.drop();
        assertEquals(TEST_MESSAGE.length(), DatagramChannel.open(StandardProtocolFamily.INET)
