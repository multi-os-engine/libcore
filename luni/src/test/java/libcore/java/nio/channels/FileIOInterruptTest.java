 ...
 *
 * <p>Using FIFOs has a few of drawbacks:
 * <ol>
...
public class FileIOInterruptTest extends TestCase {
  ...
  private native static void mkfifo(String path) throws ErrnoException;
  ...
  // This test fails on the RI and on Android: interrupt() does not wake up a blocking
  // FileInputStream.read() call. The code is included for documentation and reference.
  //    public void testStreamRead_exceptionOnInterrupt() throws Exception {
  //        FifoWriter fifoWriter = new FifoWriter(fifoFile);
  //        fifoWriter.start();
  //        FileInputStream fis = new FileInputStream(fifoFile);
  //
  ...
  //        // Test for expected behavior in the reader thread.
  //        waitToDie(streamReaderThread);
  //        assertSame(ClosedByInterruptException.class, streamReader.ioe.getClass());
  //        assertTrue(streamReader.wasInterrupted);
  //
  //        fifoWriter.tidyUp();
  //    }
  ...
  // This test fails on the RI and on Android: interrupt() does not wake up a blocking
  // FileInputStream.write() call. The code is included for documentation and reference.
  //  public void testStreamWrite_exceptionOnInterrupt() throws Exception {
  //      FifoReader fifoReader = new FifoReader(fifoFile);
  //      fifoReader.start();
  //      FileOutputStream fos = new FileOutputStream(fifoFile);
  //
  ...
  //      // Test for expected behavior in the writer thread.
  //      waitToDie(streamWriterThread);
  //      assertSame(ClosedByInterruptException.class, streamWriter.ioe.getClass());
  //      assertTrue(streamWriter.wasInterrupted);
  //
  //      fifoReader.tidyUp();
  //  }
  ...
  public void testChannelRead_exceptionOnCloseWhenBlocked() throws Exception {
    ...
    ChannelReader channelReader = new ChannelReader(fileInputChannel);
  ...
  public void testChannelRead_exceptionOnInterrupt() throws Exception {
    ...
    ChannelReader channelReader = new ChannelReader(fileChannel);
    Thread channelReaderThread = createAndStartThread("ChannelReader", channelReader);
  ...
  public void testChannelWrite_exceptionOnCloseWhenBlocked() throws Exception {
    ...
    ChannelWriter channelWriter = new ChannelWriter(fileOutputChannel);
  ...
  public void testChannelWrite_exceptionOnInterrupt() throws Exception {
    ...
    ChannelWriter channelWriter = new ChannelWriter(fileChannel);
    Thread channelWriterThread = createAndStartThread("ChannelWriter", channelWriter);
  ...
  private static class ChannelReader implements Runnable {
    ...
    ChannelReader(FileChannel channel) {
  ...
  private static class ChannelWriter implements Runnable {
    ...
    ChannelWriter(FileChannel channel) {
      this.channel = channel;
    }
    ...
    public void run() {
      ...
      // buffer is typically 64k).
      while (true) {
        // Make the buffer look non-empty.
