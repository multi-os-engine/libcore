...
public class PathTest extends TestCase {
    ...
    private static final Path testPath = Paths.get("testDir");
    ...
    public void test_getFileSystem() {
        assertTrue(testPath.getFileSystem().provider() instanceof
    ...
    public void test_getRoot() {
        assertEquals(Paths.get("/"), testPath.toAbsolutePath().getRoot());
        assertNull(testPath.getRoot());
    }

    public void test_getFileName() {
        assertEquals(testPath, testPath.getFileName());
        assertEquals(testPath, testPath.toAbsolutePath().getFileName());
        assertNull(testPath.getRoot());
    ...
    public void test_startsWith$String() {
        ...
        assertTrue(testPath.toAbsolutePath().startsWith("/"));
    ...
    public void test_startsWith$Path() {
        ...
        assertTrue(testPath.toAbsolutePath().startsWith(Paths.get("/")));
    ...
    public void test_relativize() {
        ...
        try {
            assertEquals(Paths.get(""), p1.relativize(p2.toAbsolutePath()));
            fail();
        } catch (IllegalArgumentException expected) {}

        try {
            assertEquals(Paths.get(""), p1.toAbsolutePath().relativize(p2));
            fail();
        } catch (IllegalArgumentException expected) {}
    }

    public void test_toURI() throws URISyntaxException {
        assertEquals(new URI("file://" + testPath.toAbsolutePath().toString()), testPath.toUri());
    }

    public void test_toAbsolutePath() {
        assertFalse(testPath.isAbsolute());
        assertTrue(testPath.toAbsolutePath().isAbsolute());
    }
    ...
    public void test_toRealPath() throws IOException {
        ...
        assertEquals(testPath.toAbsolutePath(), testPath.toRealPath());

        Path dir = Paths.get("dir1/dir2");
        Path file = Paths.get("dir1/dir2/../../file");
        Files.createDirectories(dir);
        Files.createFile(file);
        assertEquals(file.normalize().toAbsolutePath(), file.toRealPath());

        // Sym links.
        Path symLink = Paths.get("symlink");
        Files.createSymbolicLink(symLink, testPath);
        assertEquals(testPath.toAbsolutePath(), symLink.toRealPath());
        assertEquals(symLink.toAbsolutePath(), symLink.toRealPath(LinkOption.NOFOLLOW_LINKS));
    }

    public void test_toFile() {
        File file = testPath.toFile();
        assertEquals(testPath.toAbsolutePath().toString(), file.getAbsolutePath());
    }
    ...
    public void test_register$WatchService$WatchEvent_Kind() throws IOException,
        ...
        Path file = Paths.get("directory/file");
    ...
    public void test_register$WatchService$WatchEvent_Kind_Exception() throws IOException {
        WatchService watchService = FileSystems.getDefault().newWatchService();
        Path directory = Paths.get("directory1");
    ...
    public void test_toString() {
        Path p1 = Paths.get("f1/f2/f3");
        assertEquals("f1/f2/f3", p1.toString());
