...
public class Base64Test extends TestCase {
  ...
  // Test the various methods on Encoder that encode. Assumes all of them basically do the same
  // thing as tested extensively above but checks for quirks around parameters.
  public void testEncoder_encodeMethods() {
    ...
    // Test various types of byte buffer.

    ByteBuffer inputBuffer = ByteBuffer.wrap(input);
    testEncode_byteBuffer(encoder, input, encodedBytes, inputBuffer);

    inputBuffer = ByteBuffer.allocate(input.length);
  ...
  private static void testEncode_byteBuffer(
      Base64.Encoder encoder, byte[] input, byte[] encodedBytes, ByteBuffer inputBuffer) {
    assertEquals(0, inputBuffer.position());
    assertEquals(input.length, inputBuffer.remaining());
    assertEquals(input.length, inputBuffer.limit());

    ByteBuffer encodedBuffer = encoder.encode(inputBuffer);

    assertEquals(input.length, inputBuffer.position());
    assertEquals(0, inputBuffer.remaining());
    assertEquals(input.length, inputBuffer.limit());
    assertEquals(0, encodedBuffer.position());
    assertEquals(encodedBytes.length, encodedBuffer.remaining());
    assertEquals(encodedBytes.length, encodedBuffer.limit());
  }

  // Test the various methods on Decoder that decode. Assumes all of them basically do the same
  // thing as tested extensively above but checks for quirks around parameters.
  public void testDecoder_decodeMethods() {
    ...
    ByteBuffer inputBuffer = ByteBuffer.wrap(input);
    testDecode_byteBuffer(decoder, input, decodedBytes, inputBuffer);
  ...
  private static void testDecode_byteBuffer(
      Base64.Decoder decoder, byte[] input, byte[] decodedBytes, ByteBuffer inputBuffer) {
    assertEquals(0, inputBuffer.position());
    assertEquals(input.length, inputBuffer.remaining());
    assertEquals(input.length, inputBuffer.limit());
    ...
    assertEquals(decodedBytes.length, decodedBuffer.remaining());
    assertEquals(decodedBytes.length, decodedBuffer.limit());
  }
