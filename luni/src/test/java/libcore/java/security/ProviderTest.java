...
public class ProviderTest extends TestCase {
    ...
    @SuppressWarnings("serial")
    public void testProviderService_supportsParameter_KnownService_Failure() throws Exception {
        Provider provider = new MockProvider("MockProvider") {
            public void setup() {
                put("Cipher.FOO", MockCipherSpi.class.getName());
            }
        };

        Security.addProvider(provider);
        try {
            Provider.Service service = provider.getService("Cipher", "FOO");
            try {
                service.supportsParameter(new Object());
