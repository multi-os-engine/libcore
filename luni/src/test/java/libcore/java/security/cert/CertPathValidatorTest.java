...
public class CertPathValidatorTest extends TestCase {
    ...
    private void runOCSPStapledTest(CertificateStatus certStatus, final boolean goodStatus)
            throws Exception {
        X509Certificate serverCert = (X509Certificate) TestKeyStore.getServer()
        ...
        PKIXParameters params = new PKIXParameters(
                Collections
                        .singleton(
                                new TrustAnchor(
                                        (X509Certificate) TestKeyStore.getRootCa()
                                                .getPrivateKey("RSA", "RSA").getCertificate(),
                                        null)));
        ...
        chain.add((X509Certificate) TestKeyStore.getIntermediateCa().getPrivateKey("RSA", "RSA")
                .getCertificate());
