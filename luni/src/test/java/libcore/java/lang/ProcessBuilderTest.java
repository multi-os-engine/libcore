...
public class ProcessBuilderTest extends AbstractResourceLeakageDetectorTestCase {
    ...
    public void testRedirectInherit() throws Exception {
        ...
        try {
            ...
            int childId = Integer.parseInt(matcher.group(1));

            assertEquals(getFdInode(parentPid, 0), getFdInode(childId, 0)); // stdin
            assertEquals(getFdInode(parentPid, 1), getFdInode(childId, 1)); // stdout
            assertEquals(getFdInode(parentPid, 2), getFdInode(childId, 2)); // stderr
        } finally {
            process.destroy();
        }
    }

    private long getFdInode(int processId, int fd) throws ErrnoException {
