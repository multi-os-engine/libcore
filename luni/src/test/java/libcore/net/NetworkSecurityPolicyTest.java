...
public class NetworkSecurityPolicyTest extends TestCase {
    ...
    public void testCleartextTrafficPolicyWithFtpURLConnection() throws Exception {
        ...
        try (CapturingServerSocket server = new CapturingServerSocket(serverReplyOnConnect)) {
            ...
            assertDataTransmittedByClient(server);
        }
        ...
        try (CapturingServerSocket server = new CapturingServerSocket(serverReplyOnConnect)) {
            ...
            assertNoDataTransmittedByClient(server);
        }
    }
    ...
    private static void assertDataTransmittedByClient(CapturingServerSocket server) throws Exception {
        byte[] firstChunkFromClient =
                server.getFirstReceivedChunkFuture().get(2, TimeUnit.SECONDS);
        if ((firstChunkFromClient == null) || (firstChunkFromClient.length == 0)) {
            fail("Client did not transmit any data to server");
        }
    }

    private static void assertNoDataTransmittedByClient(CapturingServerSocket server) throws Exception {
        byte[] firstChunkFromClient;
        try {
            firstChunkFromClient = server.getFirstReceivedChunkFuture().get(2, TimeUnit.SECONDS);
