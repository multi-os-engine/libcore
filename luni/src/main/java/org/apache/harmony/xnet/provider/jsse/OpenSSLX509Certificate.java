...
public class OpenSSLX509Certificate extends X509Certificate {
    ...
    public static OpenSSLX509Certificate fromX509DerInputStream(InputStream is)
        ...
        try {
            return new OpenSSLX509Certificate(NativeCrypto.d2i_X509_bio(bis.getBioContext()));
        } catch (Exception e) {
    ...
    public static List<OpenSSLX509Certificate> fromPkcs7DerInputStream(InputStream is)
        ...
        final int[] certRefs;
        try {
            certRefs = NativeCrypto.d2i_PKCS7_bio(bis.getBioContext());
        } catch (Exception e) {
            throw new CertificateEncodingException(e);
        } finally {
            NativeCrypto.BIO_free(bis.getBioContext());
        ...
        final OpenSSLX509Certificate certs[] = new OpenSSLX509Certificate[certRefs.length];
        for (int i = 0; i < certRefs.length; i++) {
            certs[i] = new OpenSSLX509Certificate(certRefs[i]);
        }
    ...
    public static OpenSSLX509Certificate fromX509PemInputStream(InputStream is)
        ...
        try {
            return new OpenSSLX509Certificate(NativeCrypto.PEM_read_bio_X509_AUX(bis.getBioContext()));
        } catch (Exception e) {
            throw new CertificateEncodingException(e);
        } finally {
            NativeCrypto.BIO_free(bis.getBioContext());
    ...
    private void verifyOpenSSL(OpenSSLKey pkey) throws CertificateException, NoSuchAlgorithmException,
    ...
    public void verify(PublicKey key) throws CertificateException, NoSuchAlgorithmException,
            InvalidKeyException, NoSuchProviderException, SignatureException {
        OpenSSLKey pkey = null;
        if (key instanceof OpenSSLRSAPublicKey) {
            pkey = ((OpenSSLRSAPublicKey) key).getOpenSSLKey();
        } else if (key instanceof OpenSSLDSAPublicKey) {
            pkey = ((OpenSSLDSAPublicKey) key).getOpenSSLKey();
        } else if (key instanceof OpenSSLECPublicKey) {
            pkey = ((OpenSSLECPublicKey) key).getOpenSSLKey();
        }
