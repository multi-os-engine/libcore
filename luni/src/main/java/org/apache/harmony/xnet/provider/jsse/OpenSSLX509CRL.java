...
public class OpenSSLX509CRL extends X509CRL {
    ...
    private OpenSSLX509CRL(int ctx) {
        mContext = ctx;
    }
    ...
    public static OpenSSLX509CRL fromX509DerInputStream(InputStream is) throws ParsingException {
        ...
        try {
            final int crlCtx = NativeCrypto.d2i_X509_CRL_bio(bis.getBioContext());
            if (crlCtx == 0) {
                return null;
            }
    ...
    public static OpenSSLX509CRL fromX509PemInputStream(InputStream is) throws ParsingException {
        ...
        try {
            final int crlCtx = NativeCrypto.PEM_read_bio_X509_CRL(bis.getBioContext());
            if (crlCtx == 0) {
                return null;
            }
            return new OpenSSLX509CRL(crlCtx);
    ...
    public Set<String> getCriticalExtensionOIDs() {
        return new HashSet<String>(Arrays.asList(NativeCrypto.get_X509_CRL_ext_oids(mContext, 1)));
    }
    ...
    public Set<String> getNonCriticalExtensionOIDs() {
        return new HashSet<String>(Arrays.asList(NativeCrypto.get_X509_CRL_ext_oids(mContext, 0)));
    }

    @Override
    public boolean hasUnsupportedCriticalExtension() {
        final String[] criticalOids = NativeCrypto.get_X509_CRL_ext_oids(mContext, 1);
        for (String oid : criticalOids) {
            int extensionRef = NativeCrypto.X509_CRL_get_ext(mContext, oid);
    ...
    public X509CRLEntry getRevokedCertificate(BigInteger serialNumber) {
        int revokedRef = NativeCrypto.X509_CRL_get0_by_serial(mContext, serialNumber.toByteArray());
        if (revokedRef == 0) {
            return null;
        }
    ...
    public X509CRLEntry getRevokedCertificate(X509Certificate certificate) {
        if (certificate instanceof OpenSSLX509Certificate) {
            ...
            int x509RevokedRef = NativeCrypto.X509_CRL_get0_by_cert(mContext,
                    osslCert.getContext());
    ...
    public Set<? extends X509CRLEntry> getRevokedCertificates() {
        final int[] entryRefs = NativeCrypto.X509_CRL_get_REVOKED(mContext);

        final Set<OpenSSLX509CRLEntry> crlSet = new HashSet<OpenSSLX509CRLEntry>();
        for (int entryRef : entryRefs) {
    ...
    public boolean isRevoked(Certificate cert) {
        ...
        int x509RevokedRef = NativeCrypto.X509_CRL_get0_by_cert(mContext, osslCert.getContext());
