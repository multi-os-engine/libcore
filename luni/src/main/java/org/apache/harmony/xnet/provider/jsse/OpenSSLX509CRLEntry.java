...
public class OpenSSLX509CRLEntry extends X509CRLEntry {
    ...
    OpenSSLX509CRLEntry(int ctx) {
        mContext = ctx;
    }

    @Override
    public Set<String> getCriticalExtensionOIDs() {
        String[] critOids = NativeCrypto.get_X509_REVOKED_ext_oids(mContext, 1);
        ...
        if ((critOids.length == 0)
                && (NativeCrypto.get_X509_REVOKED_ext_oids(mContext, 0).length == 0)) {
            return null;
        }
    ...
    public Set<String> getNonCriticalExtensionOIDs() {
        String[] critOids = NativeCrypto.get_X509_REVOKED_ext_oids(mContext, 0);
         ...
         */
        if ((critOids.length == 0)
                && (NativeCrypto.get_X509_REVOKED_ext_oids(mContext, 1).length == 0)) {
            return null;
        }
    ...
    public boolean hasUnsupportedCriticalExtension() {
        final String[] criticalOids = NativeCrypto.get_X509_REVOKED_ext_oids(mContext, 1);
        for (String oid : criticalOids) {
            int extensionRef = NativeCrypto.X509_REVOKED_get_ext(mContext, oid);
    ...
    public boolean hasExtensions() {
        return (NativeCrypto.get_X509_REVOKED_ext_oids(mContext, 0).length != 0)
                || (NativeCrypto.get_X509_REVOKED_ext_oids(mContext, 1).length != 0);
    }
