...
public final class OpenSSLECDHKeyAgreement extends KeyAgreementSpi {
    ...
    public Key engineDoPhase(Key key, boolean lastPhase) throws InvalidKeyException {
        ...
        OpenSSLKey openSslPublicKey =
                ((OpenSSLKeyHolder) translateKeyToOpenSslKey(key)).getOpenSSLKey();
    ...
    protected void engineInit(Key key, SecureRandom random) throws InvalidKeyException {
        ...
        Key openSslKey = translateKeyToOpenSslKey(key);
        mOpenSslPrivateKey = ((OpenSSLKeyHolder) openSslKey).getOpenSSLKey();
        mExpectedResultLength =
                (((ECKey) openSslKey).getParams().getCurve().getField().getFieldSize() + 7) / 8;
    }
    ...
    private static Key translateKeyToOpenSslKey(Key key) throws InvalidKeyException {
        try {
            return KeyFactory.getInstance("EC", OpenSSLProvider.PROVIDER_NAME).translateKey(key);
        } catch (Exception e) {
