...
public final class Locale implements Cloneable, Serializable {
     ...
     * when an invalid extension is encountered. Returns the index of the last
     * parsable element of {@code subtags}.
     */
    private static int extractExtensions(String[] subtags, int startIndex, int endIndex,
            Map<Character, String> extensions) {
        int privateUseExtensionIndex = -1;
        int lastExtensionStartIndex = -1;
        ...
        for (; i < endIndex; i++) {
            ...
            if (subtag.length() == 1) {
                ...
                if (lastExtensionStartIndex != -1) {
                    if ((i - 1) - (lastExtensionStartIndex + 1) == 0) {
                        return lastExtensionStartIndex;
                    }

                    final String key = subtags[lastExtensionStartIndex];
                    if (extensions.containsKey(key.charAt(0))) {
                        return lastExtensionStartIndex;
                    }

                    final String value = concatenateRange(subtags, lastExtensionStartIndex + 1, i);
                    System.err.println("Emitting previous extension<" + key + "," + value + ">");
            ...
            } else if (lastExtensionStartIndex != -1) {
                // We must have encountered a valid key in order to start parsing
                // its subtags.
                final boolean isParsingPrivateUse = (privateUseExtensionIndex != -1) &&
                        (lastExtensionStartIndex == privateUseExtensionIndex);
                if (!isValidBcp47Alphanum(subtag, isParsingPrivateUse ? 1 : 2, 8)) {
                    return i;
                }
            } else {
                return i;
        ...
        if (lastExtensionStartIndex != -1) {
            if ((i - 1) == lastExtensionStartIndex) {
                return lastExtensionStartIndex;
            }
    ...
    private static Locale forLanguageTag(/* @Nonnull */ String tag, boolean strict) {
        ...
        if (extensions.containsKey('u')) {
            ...
            parseUnicodeExtension(extensions.get('u').split("-"),
