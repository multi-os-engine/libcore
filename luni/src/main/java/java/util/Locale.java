...
public final class Locale implements Cloneable, Serializable {
    ...
    // If the country field is valid then we can safely call through to ULocale.getDisplayCountry()
    // and getDisplayVariant. When the country is malformed, then it is appended to the variant
    // in ULocale so in those cases we just return the country and variant codes.
    ...
    public synchronized static void setDefault(Locale locale) {
        ...
        if (languageTag.startsWith(UNDETERMINED_LANGUAGE) &&
                ((languageTag.length() == 3) || (languageTag.indexOf("-") == 4))) {
