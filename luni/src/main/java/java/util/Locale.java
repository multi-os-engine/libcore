...
public final class Locale implements Cloneable, Serializable {
     ...
     */
    private transient final boolean fromBuilder;
    ...
    public Locale(String language) {
        this(language, "", "", "", Collections.EMPTY_SET, Collections.EMPTY_MAP,
                Collections.EMPTY_MAP, false /* from builder */);
    }
    ...
    public String toLanguageTag() {
        if (cachedLanguageTag == null) {
            cachedLanguageTag = toLanguageTagImpl();
        }
     ...
     */
    private String toLanguageTagImpl() {
        ...
        if (fromBuilder) {
            ...
            // in the builder, but we would like to use hyphens in the BCP-47
            // language tag.
        ...
        // We now need to perform a couple of mandatory transformations:
        // "" must be
        if (language.isEmpty()) {
            language = "und";
        }
        ...
        // Language (mandatory, "und" if ill formed).
        sb.append(language);

        // Script (optional, omitted if empty).
        if (!scriptCode.isEmpty()) {
            sb.append('-');
            sb.append(scriptCode);
        }

        // Region (optional, omitted if empty).
        if (!region.isEmpty()) {
            sb.append('-');
            sb.append(region);
        }

        // Variant (optional, omitted if empty).
        if (!variant.isEmpty()) {
            sb.append('-');
            sb.append(variant);
        }
    ...
    private static String[] splitIllformedVariant(String variant) {
        ...
        split[1] = concatenateRange(subTags, firstIllformedSubtag,
                firstInvalidSubtag);
