 ...
 *
 * <p>Note that Java uses several deprecated two-letter ISO 639-1 codes. The Hebrew ("he") language
 * code is rewritten as "iw", Indonesian ("id") as "in", and Yiddish ("yi") as "ji". This
 * rewriting happens even if you construct your own {@code Locale} object, not just for
 * instances returned by the various lookup methods. However, {@code Locale#toLanguageTag} will
 * convert the deprecated codes to their modern equivalents in the returned language tag.
 *
...
public final class Locale implements Cloneable, Serializable {
    ...
    /**
     * Lazily created ULocale object corresponding to the values in this Locale object.
     */
    private volatile transient ULocale cachedULocale;

    /**
     * Constructs a ULocale from this Locale object. This method must only be called from things
     * that ULocale does not depend on (e.g. everything apart from the Locale constructors,
     * static initializers).
     */
    ...
    // If the country field is valid according to ISO 3166-1 or UN M.49, then we can safely call
    // through to ULocale.getDisplayCountry() and getDisplayVariant. When the country is malformed,
    // then it is appended to the variant in ULocale so in those cases we just return the country
    // and variant codes.
    ...
    private Locale(boolean hasValidatedFields, String lowerCaseLanguageCode,
        ...
        this.hasValidCountry = hasValidatedFields;
    }
    ...
    public Locale(String language, String country, String variant, String scriptCode,
        ...
        hasValidCountry = isValidIcuCountryCode(country);
     ...
     *
     * If this locale uses the deprecated ISO 639-1 language codes for Hebrew ("iw"), Yiddish ("ji")
     * or Indonesian ("in"), they will be converted to "he", "yi" or "id" respectively.
     *
     * @since 1.7
     */
    ...
    public synchronized static void setDefault(Locale locale) {
        ...
        // If the language is undetermined, then discard the other information. Matches tags
        // which start with und and are only 3 characters long (i.e. "und") or first tag is und
        // (i.e. "und-variant...").
    ...
    private void readObject(ObjectInputStream stream) throws IOException, ClassNotFoundException {
        ...
        hasValidCountry = isValidIcuCountryCode(countryCode);
    }
    ...
    /**
     * Map modern language codes to their deprecated counterparts.
     * Hebrew: "he" -> "iw",
     * Indonesian "id" -> "in",
     * Yiddish "yi" -> "ji".
    ...
    // Returns true if the country code is valid according to BCP47 / UN M.49. Being valid in this
    // sense means that we can call methods relating to getDisplay* in ULocale.
    private static boolean isValidIcuCountryCode(String country) {
        return country.length() == 0
                || isValidBcp47Alpha(country, 2, 2)  // Alpha 2 (ISO 3166-1)
                || isUnM49AreaCode(country);  // Digit 3 (UN M.49)
    }
}
