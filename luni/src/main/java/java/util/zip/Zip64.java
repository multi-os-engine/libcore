...
public class Zip64 {
    ...
    public static ZipFile.EocdRecord parseZip64EocdRecord(RandomAccessFile raf,
        ...
        if (numEntries != totalNumEntries || diskNumber != 0 || diskWithCentralDirStart != 0) {
            throw new ZipException("Spanned archives not supported :" +
                    ...
                    ", diskNumber=" + diskNumber + ", diskWithCentralDirStart=" + diskWithCentralDirStart);
        }
    ...
    public static boolean parseZip64ExtendedInfo(ZipEntry ze, boolean fromCentralDirectory) throws ZipException {
        ...
        if (ze.extra != null && ze.extra.length > 0) {
            ...
            hasZip64ExtendedInfo = hasZip64ExtendedInfo(buf);
            ...
            if (hasZip64ExtendedInfo) {
                ...
                // The local header offset is significant only in the central directory. It makes no sense
                // within the local header itself.
        ...
        if (!hasZip64ExtendedInfo) {
            if (ze.compressedSize == MAX_ZIP_ENTRY_AND_ARCHIVE_SIZE ||
                ...
                throw new ZipException("File contains no zip64 extended information: "
                        + "name=" + ze.name + "compressedSize=" + ze.compressedSize + ", size=" + ze.size
     ...
     */
    public static void appendZip64ExtendedInfoToExtras(ZipEntry ze) {
        final byte[] output;
        // Note that we add 4 to ZIP64_EXTENDED_INFO_MIN_SIZE to account for
        // the 2 byte header and length.
        if (ze.extra == null) {
            output = new byte[ZIP64_EXTENDED_INFO_MIN_SIZE + 4];
        } else {
            // Note that we copy existing extras over and put the zip64 extended
            // info at the end. This will break in the presence of "old style" extras
            // which don't contain headers and lengths. The spec is again silent about
    ...
    /**
     * Returns {@code true} iff. {@code extras} contains a zip64 extended info record.
     * If a record is found, the buffer position points to the length field of the extended
     * info record.
     */
    private static boolean hasZip64ExtendedInfo(ByteBuffer extras) {
        boolean hasZip64ExtendedInfo = false;
        while (extras.hasRemaining()) {
            if (extras.getShort() == ZIP64_EXTENDED_INFO_HEADER_ID) {
                hasZip64ExtendedInfo = true;
                break;
            } else {
                extras.position(extras.position() + extras.getShort());
            }
        }
