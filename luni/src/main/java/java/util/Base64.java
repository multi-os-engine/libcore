...
import libcore.io.Base64DataException;
import libcore.io.Base64InputStream;
import libcore.io.Base64OutputStream;
...
public class Base64 {
    ...
    private static final Decoder URL_DECODER =
            new Decoder(
                    libcore.io.Base64.Decoder.FLAG_URL_SAFE
                    | libcore.io.Base64.Decoder.FLAG_REJECT_NON_ALPHABET_BYTES);
    private static final Encoder URL_ENCODER =
            new Encoder(
                    libcore.io.Base64.Encoder.FLAG_URL_SAFE, Encoder.LINE_LENGTH_NO_WRAP,  null);

    private static final int MIME_LINE_LENGTH = 76;
    private static final byte[] MIME_LINE_SEPARATOR = new byte[] { '\r', '\n' };
    private static final int MIME_ENCODER_FLAGS = libcore.io.Base64.Encoder.FLAG_DEFAULT;
    private static final Encoder STANDARD_MIME_ENCODER =
            new Encoder(MIME_ENCODER_FLAGS, MIME_LINE_LENGTH, MIME_LINE_SEPARATOR);
    private static final Decoder MIME_DECODER = new Decoder(libcore.io.Base64.Decoder.FLAG_DEFAULT);

    private static final Encoder BASIC_ENCODER =
            new Encoder(libcore.io.Base64.Encoder.FLAG_DEFAULT, Encoder.LINE_LENGTH_NO_WRAP, null);
    private static final Decoder BASIC_DECODER =
            new Decoder(libcore.io.Base64.Decoder.FLAG_REJECT_NON_ALPHABET_BYTES);
    ...
    public static class Decoder {
        ...
        public byte[] decode(byte[] src) {
            libcore.io.Base64.Decoder decoder = createLibcoreDecoder();
        ...
        public ByteBuffer decode(ByteBuffer buffer) {
            ...
            // TODO: Change libcore.io.Base64 to decode into a buffer
        }
        ...
        private libcore.io.Base64.Decoder createLibcoreDecoder() {
            return new libcore.io.Base64.Decoder(flags);
        }
    }
    ...
    public static class Encoder {
        ...
        private Encoder(int flags, int lineLength, byte[] lineSeparator) {
            ...
            if ((lineLength > 0 || (flags & libcore.io.Base64.Encoder.FLAG_APPEND_LINE_BREAK) > 0)
        ...
        public byte[] encode(byte[] src) {
            libcore.io.Base64.Encoder encoder = createLibcoreEncoder();
        ...
        public Encoder withoutPadding() {
            if ((flags & libcore.io.Base64.Encoder.FLAG_NO_PADDING) > 0) {
                return this;
            }
            int newFlags = flags | libcore.io.Base64.Encoder.FLAG_NO_PADDING;
        ...
        private libcore.io.Base64.Encoder createLibcoreEncoder() {
            libcore.io.Base64.Encoder encoder;
            if (lineLength == LINE_LENGTH_NO_WRAP) {
                encoder = new libcore.io.Base64.Encoder(flags, 0, null);
            } else {
                encoder = new libcore.io.Base64.Encoder(flags, lineLength, lineSeparator);
            }
            return encoder;
        }
    }
