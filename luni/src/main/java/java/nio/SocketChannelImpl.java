...
class SocketChannelImpl extends SocketChannel implements FileDescriptorChannel {
    ...
    public boolean connect(SocketAddress socketAddress) throws IOException {
        ...
        initConnected(inetSocketAddress, newStatus, true /* updateSocketState */);
     ...
     * @hide package visible for other nio classes
     */
    void initConnected(InetSocketAddress address, int status, boolean updateSocketState) {
        this.status = status;
        connectAddress = address;
        if (status == SOCKET_STATUS_CONNECTED && updateSocketState && socket != null) {
            socket.initConnected(connectAddress.getAddress(), connectAddress.getPort());
        }
    }
    ...
    public void initAccept(InetSocketAddress remoteAddress, boolean updateSocketState) {
        initBound(updateSocketState);
        initConnected(remoteAddress, SOCKET_STATUS_CONNECTED, updateSocketState);
    }
    ...
    private static class SocketAdapter extends Socket {
        ...
        SocketAdapter(PlainSocketImpl socketImpl, SocketChannelImpl channel)
            ...
            if (channel.isBound) {
                initBound(channel.localAddress, channel.localPort);
            }
            if (channel.isConnected()) {
                initConnected(
                        channel.connectAddress.getAddress(),
