...
/**
 * HeapByteBuffer implements byte[]-backed ByteBuffers.
 */
final class HeapByteBuffer extends BaseByteBuffer {
  ...
  HeapByteBuffer(int capacity, byte[] backingArray, int arrayOffset, boolean isReadOnly) {
    super(capacity, null);
  ...
  static HeapByteBuffer copy(HeapByteBuffer other, int markOfOther, boolean isReadOnly) {
    HeapByteBuffer buf = new HeapByteBuffer(other.capacity(), other.backingArray, other.arrayOffset, isReadOnly);
  ...
  @Override public ByteBuffer slice() {
    return new HeapByteBuffer(remaining(), backingArray, arrayOffset + position, isReadOnly);
  }
