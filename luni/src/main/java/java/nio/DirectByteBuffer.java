...
class DirectByteBuffer extends MappedByteBuffer {
  ...
  @Override public final boolean isValid() {
    return block.canAccess();
  }

  /** @hide */
  @Override public void allowAccess(boolean allow) {
    block.allowAccess(allow);
  }
  ...
  private void checkCanAccess() {
    if (!block.canAccess()) {
      throw new IllegalStateException("buffer was freed or is inaccessible");
    }
  }

  private void checkNotFreed() {
    if (!block.isValid()) {
      throw new IllegalStateException("buffer was freed or is invalid");
    }
  }
