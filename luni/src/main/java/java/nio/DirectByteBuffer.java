...
class DirectByteBuffer extends ByteBuffer {
  ...
  DirectByteBuffer(int address, int capacity) {
    this(MemoryBlock.wrapFromJni(address, capacity), capacity, 0);
  }

  static DirectByteBuffer copy(DirectByteBuffer other, int markOfOther) {
    DirectByteBuffer buf = new DirectByteBuffer(other.block, other.capacity(), other.offset);
  ...
  @Override public ByteBuffer slice() {
    return new DirectByteBuffer(block, remaining(), offset + position);
  }
}
