...
final class CharsetDecoderICU extends CharsetDecoder {
     ...
     * data[INVALID_BYTE_COUNT_OFFSET]  = number of invalid bytes
     */
    ...
    @Override protected void implReset() {
        ...
        data[INVALID_BYTE_COUNT_OFFSET] = 0;
        output = null;
        input = null;
        allocatedInput = null;
    ...
    @Override protected final CoderResult implFlush(CharBuffer out) {
        try {
            ...
            data[INVALID_BYTE_COUNT_OFFSET] = 0; // Make sure we don't see earlier errors.
            ...
            if (ICU.U_FAILURE(error)) {
                if (error == ICU.U_BUFFER_OVERFLOW_ERROR) {
                    ...
                    if (data[INVALID_BYTE_COUNT_OFFSET] > 0) {
                        return CoderResult.malformedForLength(data[INVALID_BYTE_COUNT_OFFSET]);
                    }
                }
            }
        ...
        try {
            ...
            if (ICU.U_FAILURE(error)) {
                if (error == ICU.U_BUFFER_OVERFLOW_ERROR) {
                    ...
                    return CoderResult.unmappableForLength(data[INVALID_BYTE_COUNT_OFFSET]);
