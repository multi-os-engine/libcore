...
class DatagramChannelImpl extends DatagramChannel implements FileDescriptorChannel {
     ...
     */
    void initBound(boolean updateSocketState) {
        SocketAddress sa;
        try {
        ...
        if (updateSocketState && socket != null) {
            socket.initBound(localAddress, localPort);
        }
    }
    ...
    synchronized public DatagramChannel connect(SocketAddress address) throws IOException {
        ...
        if (!isBound) {
            initBound(true /* updateSocketState */);
        }

        // Keep the connected state held by the channel and the socket up to date.
        initConnected(remoteAddress, remotePort, true /* updateSocketState */);
        return this;
    }
     ...
     * @hide used to sync state, non-private to avoid synthetic method
     */
    void initConnected(InetAddress remoteAddress, int remotePort, boolean updateSocketState) {
        connected = true;
        connectAddress = new InetSocketAddress(remoteAddress, remotePort);
        if (updateSocketState && socket != null) {
            socket.initConnected(remoteAddress, remotePort);
        }
    }
    ...
    synchronized public DatagramChannel disconnect() throws IOException {
        ...
        initDisconnected(true /* updateSocketState */);
     ...
     * @hide used to sync state, non-private to avoid synthetic method
     */
    void initDisconnected(boolean updateSocketState) {
        connected = false;
        connectAddress = null;
        if (updateSocketState && socket != null && socket.isConnected()) {
            socket.initDisconnected();
        }
    }
    ...
    private static class DatagramSocketAdapter extends DatagramSocket {
        ...
        DatagramSocketAdapter(DatagramSocketImpl socketimpl, DatagramChannelImpl channelImpl) {
            ...
            if (channelImpl.isBound) {
                initBound(channelImpl.localAddress, channelImpl.localPort);
            }
        ...
        public void bind(SocketAddress localAddr) throws SocketException {
            ...
            channelImpl.initBound(false /* updateSocketState */);
        }

        @Override
        public void connect(SocketAddress peer) throws SocketException {
            if (isConnected()) {
                // RI compatibility: If the Socket is already connected this neither fails nor
                // reconnects to the new address - it just does nothing.
                return;
            }
            super.connect(peer);
        ...
        public void disconnect() {
            super.disconnect();
            channelImpl.initDisconnected(false /* updateSocketState */);
        }
    }
}
