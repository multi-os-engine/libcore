...
import libcore.util.NativeAllocationRegister;
...
final class BigInt {
    ...
    private static NativeAllocationRegister nativeAlloc = new NativeAllocationRegister(
            NativeBN.freeFunction(), NativeBN.size());
    ...
    private void makeValid() {
        if (this.bignum == 0) {
            nativeAlloc.registerNativeAllocation();
            this.bignum = NativeBN.BN_new();
            nativeAlloc.registerNativeFree(this, this.bignum);
        }
    }
    ...
    private static BigInt newBigInt() {
        ...
        nativeAlloc.registerNativeFree(bi, bi.bignum);
        return bi;
    }
