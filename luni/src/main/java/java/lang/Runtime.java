...
public class Runtime {
    ...
    private String doLoad(String name, ClassLoader loader) {
        ...
        // The ClassLoader set up by frameworks/base knows the appropriate path, so we can load
        // libraries with no dependencies just fine, but an app that has multiple libraries that
        // depend on each other needed to load them in most-dependent-first order.
        // We added API to the dynamic linker so we can update the library path that it'll use for
        // the currently-running process, so we pull the desired path out of the ClassLoader here
