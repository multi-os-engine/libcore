 ...
 * collation_element('b').
 *
 * <p>The method {@link #next} returns the collation order of the next character based on the
 * comparison level of the collator. The method {@link #previous} returns the collation order
 * of the previous character based on the comparison level of the collator. The iterator moves
 * only in one direction between calls to {@link #reset}, {@link #setOffset}, or {@link #setText}.
 * That is, calls to {@code next} and {@code previous} can not be mixed.
 * Whenever {@code previous} is to be called after {@code next} or vice versa,
 * {@code reset}, {@code setOffset} or {@code setText}
 * has to be called first to reset the status, shifting current position to either the end or
 * the start of the string ({@code reset} or {@code setText}),
 * or the specified position ({@code setOffset}).
 * Hence at the next call of {@code next} or {@code previous}, the first or last collation order,
 * or collation order at the specified position will be returned.
 * If a change of direction is done without one of these calls, the result is undefined.
 */
