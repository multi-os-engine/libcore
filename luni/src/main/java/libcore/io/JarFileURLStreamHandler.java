...
/**
 * A {@link URLStreamHandler} for an open {@link JarFile}. This class avoids the need to open a jar
 * file multiple times to read resources if the jar file can be held open. The {@link URLConnection}
 * objects created are a subclass of {@link JarURLConnection}.
 */
public class JarFileURLStreamHandler extends URLStreamHandler {
  ...
  public JarFileURLStreamHandler(JarFile jarFile) {
     ...
     * names.
     */
    this.fileUri = new File(jarFile.getName()).toURI().toString();
  }

  public URL getEntryUrlOrNull(String name) {
    if (jarFile.getEntry(name) != null) {
      try {
        // TODO Should be URL encoding name?
        return new URL("jar", null, -1, fileUri + "!/" + name, this);
  ...
  protected URLConnection openConnection(URL url) throws IOException {
    return new JarEntryJarURLConnection(url, jarFile);
  }

  private static class JarEntryJarURLConnection extends JarURLConnection {
    ...
    public JarEntryJarURLConnection(URL url, JarFile jarFile) throws MalformedURLException {
      super(url);
      this.jarFile = jarFile;
      this.jarEntry = jarFile.getJarEntry(getEntryName());
      if (jarEntry == null) {
