...
public final class Base64 {
        ...
        try {
            while (pos[0] < inLength) {
                ...
                for (int j = 0; j < 4; j++) {
                    ...
                    if (pos[0] == inLength || c == PAD_AS_BYTE) {
                        ...
                        switch (j) {
                            ...
                            case 2:
                                ...
                                // We are at a pad character, consume and look for the seecond one.
                                pos[0]++;
    ...
    /**
     * Skips whitespace. Returns 0 on end of string.
     */
    private static byte getNextByte(byte[] in, int[] pos, int inLength)
        ...
        while (pos[0] < inLength
                && (c = base64AlphabetToNumericalValue(in[pos[0]])) == WHITESPACE_AS_BYTE) {
            pos[0]++;
        }
        return c;
    }
    ...
    private static byte[] checkNoTrailingAndReturn(
            ByteArrayOutputStream output, byte[] in, int i, int inLength)
                    throws InvalidBase64ByteException{
        // Allow only whitespace bytes after the padding.
        byte c;
        while (i < inLength
                && ((c = base64AlphabetToNumericalValue(in[i])) == WHITESPACE_AS_BYTE
                        || c == PAD_AS_BYTE)) {
            i++;
        }
        if (i != inLength) {
