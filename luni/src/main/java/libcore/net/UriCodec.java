...
public abstract class UriCodec {
     ...
     *  - If a char {@link #isWhitelistedOrRetained}, it is valid both escaped and unescaped.
    ...
    /**
     * Interprets a char as an hex digits, returning a number from -1 (invalid char) to 15 ('f').
     */
    ...
    /**
     * Throws URISyntax if any character in {@code uri}  or is not a character in {@code legal}.
     */
    ...
    public final String pencode(String s, Charset charset) {
        StringBuilder builder = new StringBuilder();
        appendEncoded(builder, s, charset, false);
        return builder.toString();
    }
    ...
    private void appendEncoded(
        ...
        // The encoder needs a buffer (there is no method to decode a single character)
        // Avoid creating a different buffer each time.
    ...
    private static void flushEncodingCharBuffer(
        ...
        // Underflow with empty output means OK. Still need to flush in case the encoder saves
        // internal state.
        ...
        // Use the character buffer for reading again.
        cBuffer.flip();
            ...
            switch (c) {
                case '+':
                    flushDecodingByteAccumulator(
                            builder, decoder, byteAccumulator, throwOnFailure);
                ...
                case '%':
                    ...
                    for (int j = 0; j < 2; j++) {
                        ...
                        hexValue = hexValue * 0x10 + newDigit;
                    }
                    byteAccumulator.write(hexValue);
                    break;
                default:
                    flushDecodingByteAccumulator(builder, decoder, byteAccumulator, throwOnFailure);
                    builder.append(c);
            }
        }
        flushDecodingByteAccumulator(builder, decoder, byteAccumulator, throwOnFailure);
    ...
    private static void flushDecodingByteAccumulator(
            ...
            ByteArrayOutputStream byteAccumulator,
            boolean throwOnFailure) {
        if (byteAccumulator.size() > 0) {
            try {
                builder.append(decoder.decode(
                        ByteBuffer.wrap(byteAccumulator.toByteArray())));
