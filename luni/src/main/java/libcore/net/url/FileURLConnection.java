...
public class FileURLConnection extends URLConnection {
    ...
    private static final int CONTENT_LENGTH_IDX = 3;
    private static final int LAST_MODIFIED_IDX = 5;
    ...
    public void connect() throws IOException {
        ...
        if (f.isDirectory()) {
            ...
            // use -1 for the contentLengthc
            lastModified = f.lastModified();
            headerKeysAndValues[CONTENT_TYPE_IDX] = "text/plain";
        } else {
            try {
            ...
            if (error == null) {
                length = f.length();
                lastModified = f.lastModified();
                headerKeysAndValues[CONTENT_TYPE_IDX] = getContentTypeForPlainFiles();
            } else {
                headerKeysAndValues[CONTENT_TYPE_IDX] = "content/unknown";
            }
        }

        headerKeysAndValues[CONTENT_LENGTH_IDX] = String.valueOf(length);
        headerKeysAndValues[LAST_MODIFIED_IDX] = String.valueOf(lastModified);
    ...
    public Map<String, List<String>> getHeaderFields() {
        if (headerFields == null) {
            headerFields = new TreeMap<>(HEADER_COMPARATOR);

            for (int i = 0; i < headerKeysAndValues.length; i+=2) {
                headerFields.put(headerKeysAndValues[i],
    ...
    public String getContentType() {
        ...
        final String contentTypeHeader = headerKeysAndValues[CONTENT_TYPE_IDX];
        return contentTypeHeader != null ? contentTypeHeader : "content/unknown";
    }
