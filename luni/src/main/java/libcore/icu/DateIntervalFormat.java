...
import static libcore.icu.FormatUtils.FORMAT_UTC;
...
public final class DateIntervalFormat {
  ...
  public static String formatDateRange(ULocale icuLocale, com.ibm.icu.util.TimeZone icuTimeZone,
    ...
    Calendar startCalendar = FormatUtils.createIcuCalendar(icuTimeZone, icuLocale, startMs);
    Calendar endCalendar;
    if (startMs == endMs) {
      endCalendar = startCalendar;
    } else {
      endCalendar = FormatUtils.createIcuCalendar(icuTimeZone, icuLocale, endMs);
    }
    ...
    if (startMs != endMs && endsAtMidnight &&
        ((flags & FormatUtils.FORMAT_SHOW_TIME) == 0
            || FormatUtils.dayDistance(startCalendar, endCalendar) <= 1)) {
      endCalendar.roll(Calendar.DAY_OF_MONTH, false);
    }

    String skeleton = FormatUtils.toSkeleton(startCalendar, endCalendar, flags);
