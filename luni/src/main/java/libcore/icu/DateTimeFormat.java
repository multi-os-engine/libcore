...
public class DateTimeFormat {
  ...
  static class FormatterCache
      extends BasicLruCache<String, com.ibm.icu.text.DateFormat> {
    FormatterCache() {
      super(8);
    }
  }
  ...
  public static String format(ULocale icuLocale, Calendar time, int flags, boolean capitalize) {
    String skeleton = FormatUtils.toSkeleton(time, flags);
    String key = skeleton + "\t" + icuLocale + "\t" + time.getTimeZone();
    DisplayContext displayContext = FormatUtils.getDisplayContext(capitalize);
