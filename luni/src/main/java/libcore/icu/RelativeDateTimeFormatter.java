...
import static libcore.icu.FormatUtils.FORMAT_ABBREV_ALL;
import static libcore.icu.FormatUtils.FORMAT_ABBREV_MONTH;
import static libcore.icu.FormatUtils.FORMAT_ABBREV_RELATIVE;
import static libcore.icu.FormatUtils.FORMAT_NO_YEAR;
import static libcore.icu.FormatUtils.FORMAT_NUMERIC_DATE;
import static libcore.icu.FormatUtils.FORMAT_SHOW_DATE;
import static libcore.icu.FormatUtils.FORMAT_SHOW_TIME;
import static libcore.icu.FormatUtils.FORMAT_SHOW_YEAR;
...
public final class RelativeDateTimeFormatter {
   ...
   */
  public static String getRelativeTimeSpanString(Locale locale, java.util.TimeZone tz, long time,
      long now, long minResolution, int flags, boolean capitalize) {
    if (locale == null) {
    ...
    com.ibm.icu.util.TimeZone icuTimeZone = FormatUtils.icuTimeZone(tz);
  ...
  private static String getRelativeTimeSpanString(ULocale icuLocale,
    ...
    // Android has flip-flopped and been inconsistent about capitalization in the past. Now we
    // capitalize consistently.
    final DisplayContext capitalizationContext = FormatUtils.getDisplayContext(capitalize);
    ...
    if (duration < MINUTE_IN_MILLIS && minResolution < MINUTE_IN_MILLIS) {
      ...
      Calendar timeCalendar = FormatUtils.createIcuCalendar(icuTimeZone, icuLocale, time);
      ...
      if ((flags & (FORMAT_NO_YEAR | FORMAT_SHOW_YEAR)) == 0) {
        Calendar nowCalendar = FormatUtils.createIcuCalendar(icuTimeZone, icuLocale, now);
      ...
      return DateTimeFormat.format(icuLocale, timeCalendar, flags, capitalize);
    }

    synchronized (CACHED_FORMATTERS) {
      com.ibm.icu.text.RelativeDateTimeFormatter formatter =
          getFormatter(icuLocale, style, capitalizationContext);
      if (relative) {
  ...
  public static String getRelativeDateTimeString(Locale locale, java.util.TimeZone tz, long time,
    ...
    if (duration < transitionResolution) {
      ...
      dateClause = DateTimeFormat.format(icuLocale, timeCalendar, flags, true /* capitalize */);
    }
