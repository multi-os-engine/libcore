...
public class HexEncoding {
    ...
    public static char[] encode(byte[] data, int offset, int len) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            result[i << 1] = (HEX_DIGITS[(b >>> 4) & 0x0f]);
            result[(i << 1) | 0x01] = (HEX_DIGITS[b & 0x0f]);
        }
    ...
    private static int toDigit(char[] str, int offset) throws IllegalArgumentException {
        ...
        int pseudoCodePoint = (int) str[offset];
        if (pseudoCodePoint < 128) {
            // Optimized for ASCII
            int result = -1;
            if ('0' <= pseudoCodePoint && pseudoCodePoint <= '9') {
                result = pseudoCodePoint - '0';
            } else if ('a' <= pseudoCodePoint && pseudoCodePoint <= 'z') {
                result = 10 + (pseudoCodePoint - 'a');
            } else if ('A' <= pseudoCodePoint && pseudoCodePoint <= 'Z') {
                result = 10 + (pseudoCodePoint - 'A');
            }
