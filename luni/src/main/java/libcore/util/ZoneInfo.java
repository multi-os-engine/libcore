...
public final class ZoneInfo extends TimeZone {
     ...
     *
     * <p>This class contains a rewrite of various native functions that Time once relied
     * on such as mktime_tz and localtime_tz. This replacement does not support leap
     ...
     * 1901 and after 2038. This is intentional, and might be hard to change, not least because
     * using values like zero for year can indicate an invalid date and may be relied upon by
     * application code. If we moved to long the year zero would be valid.
     *
     ...
     */
    /*
     * Elements of the WallTime class are a port of Bionic's localtime.c to Java. That code was
    ...
    public static class WallTime {
        ...
        public int mktime(ZoneInfo zoneInfo) {
            ...
            try {
                ...
                // OffsetInterval; this will get us the OffsetInterval we need to be or very close.
        ...
        /**
         * Attempt to apply DST adjustments to {@code oldWallTimeSeconds} to create a
         * wall time in {@code targetInterval}.
         *
         ...
         * assertion is incorrect and is the result of some modification the caller made to a valid
         * wall time that pushed them outside of the offset interval they were in. We must correct
         * for any DST change that should have been applied when they did so.
         *
         ...
         * If {@code this.isDst == -1} it means that any transition can be used.
         *
        ...
        private Integer doWallTimeSearch(ZoneInfo zoneInfo, int initialTransitionIndex,
            ...
            // transitions in that direction. The search stops when a match is found or if clampTop
            // and clampBottom are both true.
            ...
            do {
                ...
                // Match against the OffsetInterval.
                if (mustMatchDst) {
                    ...
                    // To retain similar behavior to the old implementation: if the caller is
    ...
    private static int checkedAdd(int a, int b) throws CheckedArithmeticException {
        // Adapted from Guava LongMath.checkedAdd();
        int result = a + b;
        if (!((a ^ b) < 0 | (a ^ result) >= 0)) {
    ...
    private static int checkedSubtract(int a, int b) throws CheckedArithmeticException {
        // Adapted from Guava LongMath.checkedSubtract();
        int result = a - b;
        if (!((a ^ b) >= 0 | (a ^ result) >= 0)) {
            throw new CheckedArithmeticException();
        }
