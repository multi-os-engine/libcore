 ...
 *
 * <p>The underlying transition and offset information is stored in the arrays
 * {@link #mTransitions}, {@link #mTypes}, {@link #mOffsets} and {@link #mIsDsts}. They are grouped
 * into two pairs and each array in the pair is of the same length. The first pair
 * {@link #mTransitions} and {@link #mTypes} respectively contain the times at which the transition
 * occurs in seconds since the epoch (1970-01-01 00:00:00 UTC) and the type of the transition. The
 * transitions are in order from oldest to newest. The transition type is an index into the second
 * pair of arrays {@link #mOffsets} and {@link #mIsDsts} which respectively contain the offset (in
 * seconds) and whether the offset includes DST or not.
 *
...
public final class ZoneInfo extends TimeZone {
     ...
     * in the offset from GMT or a change in the DST.
     *
     ...
     *
     * <p>In the underlying data structure a type has more information associated with it other
     * than just the offset and whether it includes DST, e.g. it also includes an abbreviated
     * identifier for the time zone. So, it is an N-tuple (for some N) rather than simply a pair.
     * The structures are compressed to reduce space so that within each time zone each N-tuple is
     * unique. However, as this only uses two pieces of information from that the resulting pairs
     * are not unique.
     *
     * @see #mTransitions
     * @see #mOffsets
    ...
    /**
     * The offset parts of the transition types.
     *
     * <p>The offset in seconds to apply to UTC time.
     *
     * @see #mTypes
     * @see #mIsDsts
     */
    ...
    int findOffsetIndexForTimeInMilliseconds(long millis) {
        // This rounds the time in milliseconds down to the time in seconds as transition times
        // indicate the start of the time period to which they apply and so in order to find the
        // transition that applies to a specific time it is necessary for -2001 milliseconds to be
        // before -2 seconds, and 1999 milliseconds to be before 2 seconds so that the wrong
        // transition is not picked.
        //
        // e.g. if the transition time is -12 seconds, or -12,000 milliseconds and the time is
        //      -12,001, then the transition should not be active but dividing by 1000 rounds
        //      -12,001 milliseconds to -12 seconds (instead of -13 seconds).
    ...
    public static class WallTime {
        ...
        public int mktime(ZoneInfo zoneInfo) {
            ...
            final long longWallTimeSeconds = roundDownMillisToSeconds(calendar.getTimeInMillis());
