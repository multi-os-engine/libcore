...
static JNICALL jlong AlphabeticIndex_create(JNIEnv* env, jclass, jstring javaLocaleName) {
...
static JNICALL void AlphabeticIndex_destroy(JNIEnv*, jclass, jlong peer) {
  delete fromPeer(peer);
}

static JNICALL jint AlphabeticIndex_getMaxLabelCount(JNIEnv*, jclass, jlong peer) {
  icu::AlphabeticIndex* ai = fromPeer(peer);
  return ai->getMaxLabelCount();
}

static JNICALL void AlphabeticIndex_setMaxLabelCount(JNIEnv* env, jclass, jlong peer, jint count) {
  icu::AlphabeticIndex* ai = fromPeer(peer);
  UErrorCode status = U_ZERO_ERROR;
...
static JNICALL void AlphabeticIndex_addLabels(JNIEnv* env, jclass, jlong peer, jstring javaLocaleName) {
  icu::AlphabeticIndex* ai = fromPeer(peer);
  ScopedIcuLocale icuLocale(env, javaLocaleName);
  if (!icuLocale.valid()) {
    return;
...
static JNICALL void AlphabeticIndex_addLabelRange(JNIEnv* env, jclass, jlong peer,
...
static JNICALL jint AlphabeticIndex_getBucketCount(JNIEnv* env, jclass, jlong peer) {
  icu::AlphabeticIndex* ai = fromPeer(peer);
  UErrorCode status = U_ZERO_ERROR;
...
static JNICALL jint AlphabeticIndex_getBucketIndex(JNIEnv* env, jclass, jlong peer, jstring javaString) {
  icu::AlphabeticIndex* ai = fromPeer(peer);
...
static JNICALL jstring AlphabeticIndex_getBucketLabel(JNIEnv* env, jclass, jlong peer, jint index) {
  if (index < 0) {
...
static JNICALL jlong AlphabeticIndex_buildImmutableIndex(JNIEnv* env, jclass, jlong peer) {
  icu::AlphabeticIndex* ai = fromPeer(peer);
  UErrorCode status = U_ZERO_ERROR;
...
static JNICALL jint ImmutableIndex_getBucketCount(JNIEnv*, jclass, jlong peer) {
  icu::AlphabeticIndex::ImmutableIndex* ii = immutableIndexFromPeer(peer);
  return ii->getBucketCount();
}

static JNICALL jint ImmutableIndex_getBucketIndex(JNIEnv* env, jclass, jlong peer, jstring javaString) {
  icu::AlphabeticIndex::ImmutableIndex* ii = immutableIndexFromPeer(peer);
  ScopedJavaUnicodeString string(env, javaString);
  if (!string.valid()) {
    return -1;
...
static JNICALL jstring ImmutableIndex_getBucketLabel(JNIEnv* env, jclass, jlong peer, jint index) {
  icu::AlphabeticIndex::ImmutableIndex* ii = immutableIndexFromPeer(peer);
