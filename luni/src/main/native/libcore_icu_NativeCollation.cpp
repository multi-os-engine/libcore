...
// Manages a UCollationElements instance along with the uchar*
...
public:
    ...
    UErrorCode start(JNIEnv* env, jstring string, UCollator* collator) {
        ...
        if (mChars != NULL) {
            mString = (jstring) env->NewGlobalRef(string);
...
static jint NativeCollation_next(JNIEnv* env, jclass, jlong address) {
    UErrorCode status = U_ZERO_ERROR;
    int32_t result = ucol_next(toCollationElements(address)->get(), &status);
