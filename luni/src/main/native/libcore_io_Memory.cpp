...
template <typename T> static inline T get_unaligned(const T* address) {
    typedef struct unaligned { T v; } __attribute__ ((packed)) unaligned;
    const unaligned* unalignedaddress = reinterpret_cast<const unaligned*>(address);
    return unalignedaddress->v;
}

template <typename T> static inline void put_unaligned(T* address, T v) {
    typedef struct unaligned { T v; } __attribute__ ((packed)) unaligned;
    unaligned* unalignedaddress = reinterpret_cast<unaligned*>(address);
    unalignedaddress->v = v;
}
...
// byteswap 2 jshort values packed in a jint
