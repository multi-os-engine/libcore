...
static JNICALL jstring NativeNormalizer_normalizeImpl(JNIEnv* env, jclass, jstring s, jint intMode) {
...
static JNICALL jboolean NativeNormalizer_isNormalizedImpl(JNIEnv* env, jclass, jstring s, jint intMode) {
  ScopedJavaUnicodeString src(env, s);
  if (!src.valid()) {
    return JNI_FALSE;
  }
