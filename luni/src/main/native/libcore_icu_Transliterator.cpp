...
static JNICALL jlong Transliterator_create(JNIEnv* env, jclass, jstring javaId) {
...
static JNICALL void Transliterator_destroy(JNIEnv*, jclass, jlong peer) {
  delete fromPeer(peer);
}

static JNICALL jobjectArray Transliterator_getAvailableIDs(JNIEnv* env, jclass) {
  UErrorCode status = U_ZERO_ERROR;
...
static JNICALL jstring Transliterator_transliterate(JNIEnv* env, jclass, jlong peer, jstring javaString) {
