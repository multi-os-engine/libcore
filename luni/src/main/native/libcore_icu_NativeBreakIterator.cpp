...
#include "ScopedUtfChars.h"
    ...
    #define BreakIteratorFactory BreakIterator
#else
    #include <cf_break_iterator_factory.h>
    #define BreakIterator cf::BreakIterator
    #define BreakIteratorFactory cf::BreakIteratorFactory
#endif
...
template <class T>
uintptr_t MAKE_BREAK_ITERATOR_INSTANCE(JNIEnv* env, jstring javaLocaleName, T fncPointer) {
    ScopedIcuLocale icuLocale(env, javaLocaleName);
    if (!icuLocale.valid()) {
        return 0;
    }
    UErrorCode status = U_ZERO_ERROR;
    icu::BreakIterator* it = fncPointer(icuLocale.locale(), status);
    if (maybeThrowIcuException(env, "ubrk_open", status)) {
        return 0;
    }
    return reinterpret_cast<uintptr_t>(it);
}
static JNICALL jlong NativeBreakIterator_cloneImpl(JNIEnv* env, jclass, jlong address) {
  BreakIteratorAccessor it(env, address);
  return reinterpret_cast<uintptr_t>(it->clone());
}

static JNICALL void NativeBreakIterator_closeImpl(JNIEnv*, jclass, jlong address) {
  delete toBreakIterator(address);
}

static JNICALL jint NativeBreakIterator_currentImpl(JNIEnv* env, jclass, jlong address, jstring javaInput) {
  BreakIteratorAccessor it(env, address, javaInput, false);
  return it->current();
}

static JNICALL jint NativeBreakIterator_firstImpl(JNIEnv* env, jclass, jlong address, jstring javaInput) {
  BreakIteratorAccessor it(env, address, javaInput, false);
  return it->first();
}

static JNICALL jint NativeBreakIterator_followingImpl(JNIEnv* env, jclass, jlong address, jstring javaInput, jint offset) {
  BreakIteratorAccessor it(env, address, javaInput, false);
  return it->following(offset);
}

static JNICALL jlong NativeBreakIterator_getCharacterInstanceImpl(JNIEnv* env, jclass, jstring javaLocaleName) {
  return MAKE_BREAK_ITERATOR_INSTANCE(env, javaLocaleName, icu::BreakIteratorFactory::createCharacterInstance);
}

static JNICALL jlong NativeBreakIterator_getLineInstanceImpl(JNIEnv* env, jclass, jstring javaLocaleName) {
  return MAKE_BREAK_ITERATOR_INSTANCE(env, javaLocaleName, icu::BreakIteratorFactory::createLineInstance);
}

static JNICALL jlong NativeBreakIterator_getSentenceInstanceImpl(JNIEnv* env, jclass, jstring javaLocaleName) {
  return MAKE_BREAK_ITERATOR_INSTANCE(env, javaLocaleName, icu::BreakIteratorFactory::createSentenceInstance);
}

static JNICALL jlong NativeBreakIterator_getWordInstanceImpl(JNIEnv* env, jclass, jstring javaLocaleName) {
  return MAKE_BREAK_ITERATOR_INSTANCE(env, javaLocaleName, icu::BreakIteratorFactory::createWordInstance);
}

static JNICALL jboolean NativeBreakIterator_isBoundaryImpl(JNIEnv* env, jclass, jlong address, jstring javaInput, jint offset) {
  BreakIteratorAccessor it(env, address, javaInput, false);
  return it->isBoundary(offset);
}

static JNICALL jint NativeBreakIterator_lastImpl(JNIEnv* env, jclass, jlong address, jstring javaInput) {
  BreakIteratorAccessor it(env, address, javaInput, false);
  return it->last();
}

static JNICALL jint NativeBreakIterator_nextImpl(JNIEnv* env, jclass, jlong address, jstring javaInput, jint n) {
  BreakIteratorAccessor it(env, address, javaInput, false);
  if (n < 0) {
    while (n++ < -1) {
      it->previous();
...
static JNICALL jint NativeBreakIterator_precedingImpl(JNIEnv* env, jclass, jlong address, jstring javaInput, jint offset) {
  BreakIteratorAccessor it(env, address, javaInput, false);
  return it->preceding(offset);
}

static JNICALL jint NativeBreakIterator_previousImpl(JNIEnv* env, jclass, jlong address, jstring javaInput) {
  BreakIteratorAccessor it(env, address, javaInput, false);
  return it->previous();
}

static JNICALL void NativeBreakIterator_setTextImpl(JNIEnv* env, jclass, jlong address, jstring javaInput) {
