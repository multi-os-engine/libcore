...
// Buffer size of our text window.
...
// static int32_t Extract(UText*, int64_t, int64_t, UChar*, int32_t, UErrorCode*);
...
static UText* Clone(UText* dest, const UText* source, UBool deep, UErrorCode* status) {
  ...
  // for smaller strings.
  memcpy(dest->pExtra, source->chunkContents, sizeof(uint16_t) * kBufferSize);
