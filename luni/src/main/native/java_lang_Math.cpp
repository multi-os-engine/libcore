...
static jdouble Math_sqrt(JNIEnv*, jclass, jdouble a) {
     ...
     * x86 host mode.
     *
     * Math.sqrt() calls sqrt() from libm.so. In some
     * cases x86 sqrt() produces results different
     * from x86_64 sqrt() when run in host mode. It
     * also produces results different from x86 target
     * mode. Such a behavior is unallowable because
     * results must be the same regardless of mode.
     * The ieee_sqrt() is supposed to be more accurate
     * but slower. However, we may afford a bit of
     * performance degradation in host mode.
     */
    return ieee_sqrt(a);
}
#else
