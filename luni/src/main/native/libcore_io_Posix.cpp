...
static bool javaNetlinkSocketAddressToSockaddr(
    ...
    return false;
#else
    static jfieldID nlFamilyFid = env->GetFieldID(JniConstants::netlinkSocketAddressClass, "nl_family", "I");
    static jfieldID nlPidFid = env->GetFieldID(JniConstants::netlinkSocketAddressClass, "nl_pid", "I");
    static jfieldID nlGroupsFid = env->GetFieldID(JniConstants::netlinkSocketAddressClass, "nl_groups", "I");
...
static bool javaSocketAddressToSockaddr(
    ...
    if (env->IsInstanceOf(javaSocketAddress, JniConstants::netlinkSocketAddressClass)) {
        if (!javaNetlinkSocketAddressToSockaddr(env, javaSocketAddress, ss, sa_len)) {
            // Exception already thrown.
            return false;
        }
    } else if (env->IsInstanceOf(javaSocketAddress, JniConstants::inetSocketAddressClass)) {
        if (!javaInetSocketAddressToSockaddr(env, javaSocketAddress, ss, sa_len)) {
            // Exception already thrown.
            return false;
        }
    } else {
