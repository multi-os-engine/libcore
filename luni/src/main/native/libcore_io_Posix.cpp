...
static jint Posix_writeBytes(JNIEnv* env, jobject, jobject javaFd, jbyteArray javaBytes, jint byteOffset, jint byteCount) {
     ...
     * write(2) NOTES section. The case with a >0 result and where not as many bytes as requested
     * were written is treated as special: InterruptedIOException is thrown if the file descriptor
     * is closed. Closure indicates an asynchronous close (perhaps as the result of a
