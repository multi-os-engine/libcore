...
static jboolean ICU_initLocaleDataNative(JNIEnv* env, jclass, jstring javaLanguageTag, jobject localeData) {
    ...
    // If we've found abbreviated AM/PM markers for this locale ("am", "pm" etc.), we
    // use them instead of using the regular AM/PM markers. Abbreviated markers are
    // missing in most locales and are the same as regular markers in many others. In
    // the few locales that they do differ, they seem like better choices in
    // most UX contexts. Note that this is a deviation from ICU4J / ICU4C formatters.
    //
    // See bug 21015990
