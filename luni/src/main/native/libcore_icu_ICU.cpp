...
bool mapIcuData(std::string& path) {
...
void register_libcore_icu_ICU(JNIEnv* env) {
    ...
    dataPath += "/misc/zoneinfo/current/icu/";
    dataPath += "tzdata_";
    dataPath += U_ICUDATA_NAME;
    dataPath += ".dat";

    struct stat sb;
    if (stat(dataPath.c_str(), &sb) == 0) {
        ALOGI("Timezone override file found: %s", dataPath.c_str());
        if (!mapIcuData(dataPath)) {
            // TODO Should ignore this and safely continue?
            abort();
        }
        // TODO Can we u_init here to check the data is ok?
    } else {
        ALOGI("No timezone override file found: %s", dataPath.c_str());
    }
