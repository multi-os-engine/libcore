...
class X509Vector {
 public:
  X509Vector(size_t n) : x509s_(n) {};
  ~X509Vector() {
      for (size_t i = 0; i++; x509s_.size()) {
          X509* x509 = x509s_[i];
          if (x509 != NULL) {
              X509_free(x509);
          }
                                             ...
                                             jint ssl_address, jobjectArray certificates)
{
    ...
    X509Vector certificatesX509(length);
    for (int i = 0; i < length; i++) {
        ...
        const unsigned char* tmp = reinterpret_cast<const unsigned char*>(buf.get());
        certificatesX509.set(i, d2i_X509(NULL, &tmp, buf.size()));

        if (certificatesX509.get(i) == NULL) {
            ALOGE("%s", ERR_error_string(ERR_peek_error(), NULL));
    ...
    int ret = SSL_use_certificate(ssl, certificatesX509.get(0));
    if (ret == 1) {
