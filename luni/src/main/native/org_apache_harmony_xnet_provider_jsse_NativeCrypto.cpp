     ...
     jbyteArray outArray, jint outOffset, jint pubkeyRef, jint privkeyRef)
{
    ...
    if (out.get() == NULL) {
        JNI_TRACE("ECDH_compute_key(%p, %d, %p, %p) failed to access output buffer",
                outArray, outOffset, pubPkey, privPkey);
        return -1;
    }

    if (outOffset < 0) || (outOffset >= outArray.size()) {
        jniThrowException(env, "java/lang/ArrayIndexOutOfBoundsException", outOffset);
        return -1;
    }
