...
static long NativeCrypto_X509_get_version(JNIEnv*, jclass, jint x509Ref) {
    ...
    JNI_TRACE("X509_cmp_current_time(%p) => %ld", x509, version);
    return version;
}
...
static jbyteArray NativeCrypto_X509_get_serialNumber(JNIEnv* env, jclass, jint x509Ref) {
    ...
    if (serialBn.get() == NULL) {
        JNI_TRACE("X509_get_serialNumber(%p) => threw exception", x509);
        return NULL;
    }
    ...
    JNI_TRACE("X509_get_serialNumber(%p) => %p", x509, serialArray.get());
...
static jbyteArray NativeCrypto_get_X509_cert_info_enc(JNIEnv* env, jclass, jint x509Ref) {
    ...
    if (x509 == NULL) {
        jniThrowNullPointerException(env, "x509 == null");
        JNI_TRACE("get_X509_cert_info_enc(%p) => x509 == null", x509);
        return NULL;
    }
...
static jbyteArray NativeCrypto_get_X509_signature(JNIEnv* env, jclass, jint x509Ref) {
    ...
    ScopedLocalRef<jbyteArray> signatureArray(env, env->NewByteArray(x509->signature->length));
    if (env->ExceptionCheck()) {
        JNI_TRACE("get_X509_signature(%p) => threw exception", x509);
        return NULL;
    }

    ScopedByteArrayRW signatureBytes(env, signatureArray.get());
    if (signatureBytes.get() == NULL) {
        JNI_TRACE("get_X509_signature(%p) => using byte array failed", x509);
        return NULL;
    }

    memcpy(signatureBytes.get(), x509->signature->data, x509->signature->length);

    JNI_TRACE("get_X509_signature(%p) => %p (%d bytes)", x509, signatureArray.get(),
...
static jint NativeCrypto_PEM_read_bio_X509_AUX(JNIEnv* env, jclass, jint bioRef) {
    BIO* bio = reinterpret_cast<BIO*>(static_cast<uintptr_t>(bioRef));
    JNI_TRACE("PEM_read_bio_X509_AUX(%p)", bio);
    ...
    if (x == NULL) {
        ...
        JNI_TRACE("PEM_read_bio_X509_AUX(%p) => threw exception", bio);
        return 0;
    }

    JNI_TRACE("PEM_read_bio_X509_AUX(%p) => %p", bio, x);
    return static_cast<jint>(reinterpret_cast<uintptr_t>(x));
}

static jintArray PKCS7_to_X509Array(JNIEnv* env, PKCS7* pkcs7)
{
    ScopedLocalRef<jintArray> x509_array(env, NULL);
    ...
    if (PKCS7_type_is_signed(pkcs7)) {
        ...
        JNI_TRACE("PKCS7_to_X509Array(%p) => unknown PKCS7 type", pkcs7);
        return NULL;
    }

    size_t size = sk_X509_num(certs);
    x509_array.reset(env->NewIntArray(size));
    ScopedIntArrayRW x509s(env, x509_array.get());
    for (size_t i = 0; i < size; i++) {
        X509* cert = sk_X509_value(certs, i);
        x509s[i] = static_cast<jint>(reinterpret_cast<uintptr_t>(X509_dup(cert)));
    }

    JNI_TRACE("PKCS7_to_X509Array(%p) => %p [size=%d]", pkcs7, x509_array.get(), size);
    return x509_array.release();
}

static jintArray NativeCrypto_PEM_read_bio_PKCS7(JNIEnv* env, jclass, jint bioRef) {
    BIO* bio = reinterpret_cast<BIO*>(static_cast<uintptr_t>(bioRef));
    ...
    if (pkcs7.get() == NULL) {
        throwExceptionIfNecessary(env, "PEM_read_bio_PKCS7");
        JNI_TRACE("PEM_read_bio_PKCS7(%p) => threw exception", bio);
        return 0;
    }

    return PKCS7_to_X509Array(env, pkcs7.get());
}

static jintArray NativeCrypto_d2i_PKCS7_bio(JNIEnv* env, jclass, jint bioRef) {
    BIO* bio = reinterpret_cast<BIO*>(static_cast<uintptr_t>(bioRef));
    JNI_TRACE("PEM_read_bio_PKCS7(%p)", bio);
    ...
    if (pkcs7.get() == NULL) {
        ...
        JNI_TRACE("d2i_PKCS7_bio(%p) => threw exception", bio);
        return 0;
    }
...
static jbyteArray NativeCrypto_X509_get_ext_oid(JNIEnv* env, jclass, jint x509Ref, jstring oidString) {
    X509* x509 = reinterpret_cast<X509*>(static_cast<uintptr_t>(x509Ref));
    JNI_TRACE("X509v3_get_ext_by_OBJ_oid(%p)", x509);
    ...
    if (asn1.get() == NULL) {
        JNI_TRACE("X509_get_ext_oid(%s) => oid conversion failed", oid.c_str());
        freeOpenSslErrorState();
        return NULL;
    }

    int extIndex = X509_get_ext_by_OBJ(x509, asn1.get(), -1);
    if (extIndex == -1) {
        JNI_TRACE("X509_get_ext_oid(%s) => ext not found", oid.c_str());
        return NULL;
    }

    X509_EXTENSION* ext = X509_get_ext(x509, extIndex);
    if (ext == NULL) {
        JNI_TRACE("X509_get_ext_oid(%s) => fetching extension failed", oid.c_str());
        return NULL;
    }
...
static jobjectArray NativeCrypto_get_X509_ext_oids(JNIEnv* env, jclass, jint x509Ref,
        jint critical) {
    X509* x509 = reinterpret_cast<X509*>(static_cast<uintptr_t>(x509Ref));
    JNI_TRACE("get_X509_ext_oids(%p, %d)", x509, critical);

    if (x509 == NULL) {
        jniThrowNullPointerException(env, "x509 == null");
        JNI_TRACE("get_X509_ext_oids(%p, %d) => x509 == null", x509, critical);
        return NULL;
    }

    int lastPos = -1;
    int count = 0;
    while ((lastPos = X509_get_ext_by_critical(x509, critical, lastPos)) != -1) {
        count++;
    }

    JNI_TRACE("get_X509_ext_oids(%p, %d) has %d entries", x509, critical, count);

    ScopedLocalRef<jobjectArray> joa(env, env->NewObjectArray(count, JniConstants::stringClass, NULL));
    if (joa.get() == NULL) {
        JNI_TRACE("get_X509_ext_oids(%p, %d) => fail to allocate result array", x509, critical);
        return NULL;
    }
    ...
    count = 0;
    while ((lastPos = X509_get_ext_by_critical(x509, critical, lastPos)) != -1) {
        X509_EXTENSION* ext = X509_get_ext(x509, lastPos);

        ScopedLocalRef<jstring> extOid(env, ASN1_OBJECT_to_OID_string(env, ext->object));
        if (extOid.get() == NULL) {
            JNI_TRACE("get_X509_ext_oids(%p) => couldn't get OID", x509);
            return NULL;
        }
    ...
    JNI_TRACE("get_X509_ext_oids(%p, %d) => success", x509, critical);
    return joa.release();
}
...
static JNINativeMethod sNativeCryptoMethods[] = {
    ...
    NATIVE_METHOD(NativeCrypto, PEM_read_bio_PKCS7, "(I)[I"),
    NATIVE_METHOD(NativeCrypto, d2i_PKCS7_bio, "(I)[I"),
