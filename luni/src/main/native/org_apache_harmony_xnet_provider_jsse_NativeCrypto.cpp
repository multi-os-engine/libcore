...
static jint NativeCrypto_EVP_PKEY_new_EC_KEY(JNIEnv* env, jclass, jint groupRef,
    ...
    Unique_EVP_PKEY pkey(EVP_PKEY_new());
    if (pkey.get() == NULL) {
        JNI_TRACE("EVP_PKEY_new_EC(%p, %p) => threw error", groupRef, keyJavaBytes);
        throwExceptionIfNecessary(env, "ENGINE_load_private_key");
        return 0;
    }
    ...
    JNI_TRACE("EVP_PKEY_new_EC(%p, %p) => %p", groupRef, keyJavaBytes, eckey.get());
    return static_cast<jint>(reinterpret_cast<uintptr_t>(eckey.release()));
}
...
static jint NativeCrypto_get_EC_GROUP_type(JNIEnv* env, jclass, jint groupRef)
{
    ...
    if (method == EC_GFp_nist_method()
        ...
        JNI_TRACE("get_EC_GROUP_type(%p) => unknown method %p", group, method);
        jniThrowRuntimeException(env, "unknown curve method");
    }
...
static void NativeCrypto_EC_POINT_set_affine_coordinates(JNIEnv* env, jclass, jint type,
        jint groupRef, jint pointRef, jbyteArray xjavaBytes, jbyteArray yjavaBytes)
{
    const EC_GROUP* group = reinterpret_cast<const EC_GROUP*>(groupRef);
    EC_POINT* point = reinterpret_cast<EC_POINT*>(pointRef);
    JNI_TRACE("EC_POINT_set_affine_coordinates(%d, %p, %p, %p, %p)", type, group, point, xjavaBytes,
            yjavaBytes);

    if (group == NULL || point == NULL) {
        JNI_TRACE("EC_POINT_set_affine_coordinates(%d, %p, %p, %p, %p) => group == null || point == null",
                type, group, point, xjavaBytes, yjavaBytes);
        jniThrowNullPointerException(env, "group == null || point == null");
        return;
    }
    ...
    JNI_TRACE("EC_POINT_set_affine_coordinates(%d, %p, %p, %p, %p) => %d", type, group, point,
            xjavaBytes, yjavaBytes, ret);
}

static jobjectArray NativeCrypto_EC_POINT_get_affine_coordinates(JNIEnv* env, jclass, jint type,
        jint groupRef, jint pointRef)
{
    const EC_GROUP* group = reinterpret_cast<const EC_GROUP*>(groupRef);
    const EC_POINT* point = reinterpret_cast<const EC_POINT*>(pointRef);
    JNI_TRACE("EC_POINT_get_affine_coordinates_GFp(%d, %p, %p)", type, group, point);
    ...
    if (ret != 1) {
        JNI_TRACE("EC_POINT_get_affine_coordinates(%d, %p, %p)", type, group, point);
    ...
    JNI_TRACE("EC_POINT_get_affine_coordinates(%d, %p, %p) => %p", type, group, point, joa);
    return joa;
}
...
static jint NativeCrypto_EC_KEY_generate_key(JNIEnv* env, jclass, jint groupRef)
{
    ...
    JNI_TRACE("EC_KEY_generate_key(%p) => %p", group, eckey.get());
    return static_cast<jint>(reinterpret_cast<uintptr_t>(eckey.release()));
}

static jbyteArray NativeCrypto_EC_KEY_get_private_key(JNIEnv* env, jclass, jint keyRef)
{
    const EC_KEY* key = reinterpret_cast<const EC_KEY*>(keyRef);
    JNI_TRACE("EC_KEY_get_private_key(%p)", key);

    const BIGNUM *privkey = EC_KEY_get0_private_key(key);

    jbyteArray privBytes = bignumToArray(env, privkey);
    if (env->ExceptionCheck()) {
        JNI_TRACE("EC_KEY_get_private_key(%p) => threw error", key);
        return NULL;
    }

    JNI_TRACE("EC_KEY_get_private_key(%p) => %p", key, privBytes);
    return privBytes;
}

static jint NativeCrypto_EC_KEY_get_public_key(JNIEnv* env, jclass, jint keyRef)
{
    const EC_KEY* key = reinterpret_cast<const EC_KEY*>(keyRef);
    JNI_TRACE("EC_KEY_get_public_key(%p)", key);

    Unique_EC_POINT dup(EC_POINT_dup(EC_KEY_get0_public_key(key), EC_KEY_get0_group(key)));
    if (dup.get() == NULL) {
        JNI_TRACE("EC_KEY_get_public_key(%p) => can't dup public key", key);
        jniThrowRuntimeException(env, "EC_POINT_dup");
        return 0;
    }

    JNI_TRACE("EC_KEY_get_public_key(%p) => %p", key, dup.get());
    return static_cast<jint>(reinterpret_cast<uintptr_t>(dup.release()));
}
...
static JNINativeMethod sNativeCryptoMethods[] = {
    ...
    NATIVE_METHOD(NativeCrypto, EC_POINT_set_affine_coordinates, "(III[B[B)V"),
    NATIVE_METHOD(NativeCrypto, EC_POINT_get_affine_coordinates, "(III)[[B"),
