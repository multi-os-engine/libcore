...
public class ThreadGroupTest extends junit.framework.TestCase {
    ...
    public void test_list() {
        ...
        // java.lang.Thread
        boolean wipeSucceeded = wipeSideEffectThreads(originalCurrent);
        if (!wipeSucceeded) {
    ...
    private static Thread[] threads(ThreadGroup parent) {
    ...
    private static boolean wipeSideEffectThreads(ThreadGroup aGroup) {
        boolean ok = true;
    ...
    private static boolean wipeThread(Thread t) {
        t.stop();
        try {
            t.join(1000);
        } catch (InterruptedException ie) {
        ...
        // The thread had plenty (subjective) of time to die so there is a problem.
        return !t.isAlive();
    }
