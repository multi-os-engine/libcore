...
public final class Daemons {
    ...
    private static class FinalizerDaemon extends Daemon {
        ...
        @Override public void run() {
            ...
            // reading the current time here, since that a kernel entry per object.  We limit fast
            // path communication with FinalizerWatchDogDaemon to what's unavoidable: A
    ...
    private static class FinalizerWatchdogDaemon extends Daemon {
        ...
        private FinalizerReference<?> waitForFinalization() {
            ...
            if (getNeedToWork() && FinalizerDaemon.INSTANCE.progressCounter.get() == startCount) {
                ...
                // single doFinalize() call.  Thus it's OK to time out.
                // finalizingObject was set just before the counter increment, which preceded
                // the doFinalize call.  Thus we are guaranteed to get the correct
                // value, unless doFinalize() just finished as we were timing out, in which case
                // we may get null or a later one.
                return FinalizerDaemon.INSTANCE.finalizingObject;
            } else {
                return null;
            }
        }
