...
public final class Class<T> implements Serializable, AnnotatedElement, GenericDeclaration, Type {
    ...
    private Method getDeclaredMethodInternal(String name, Class<?>[] args) {
        ...
        if (artMethodResult == null) {
            if (directMethods != null) {
                for (ArtMethod m : directMethods) {
                    m = Class.findOverriddenMethodIfProxy(m);
                    int modifiers = m.getAccessFlags();
                    if (Modifier.isConstructor(modifiers)) {
                        continue;
                    }
     ...
     * @hide
     */
    public static ArtMethod findOverriddenMethodIfProxy(ArtMethod method) {
      return method.getDeclaringClass().findOverriddenMethodIfProxyNative(method);
    }

    private native ArtMethod findOverriddenMethodIfProxyNative(ArtMethod method);
