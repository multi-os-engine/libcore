...
import libcore.io.JarFileURLStreamHandler;

class VMClassLoader {

    private static JarFileURLStreamHandler[] bootClassPathUrlHandlers;
    static {
      initializeUrlHandlers();
    }

    private static void initializeUrlHandlers() {
        if (bootClassPathUrlHandlers == null) {
            ...
            for (String bootClassPathEntry : bootClassPathEntries) {
                try {
                    ...
                    JarFile jarFile = new JarFile(bootClassPathEntry, false /* verify */);
                    URLStreamHandler urlStreamHandler = new JarFileURLStreamHandler(jarFile);
            ...
            bootClassPathUrlHandlers = urlStreamHandlers.toArray(
                    new JarFileURLStreamHandler[urlStreamHandlers.size()]);
        }
    }
    ...
    static URL getResource(String name) {
        for (JarFileURLStreamHandler urlHandler : bootClassPathUrlHandlers) {
