...
public final class ArtMethod {
    ...
    /** Short-cut to declaringClass.dexCache.resolvedMethod. Reserved for use by compiled code. */
    private ArtMethod[] dexCacheResolvedMethods;

    /** Short-cut to declaringClass.dexCache.resolvedTypes. Reserved for use by compiled code. */
    ...
    ArtMethod findOverriddenMethodIfProxy() {
        ...
        if (declClass.isProxy()) {
            ...
            // TODO: How to retrieve the overridden class? Is the dexCacheResolvedTypes really
            // the only reference back from the proxy method to the overridden method?
            //   return declClass.getOverriddenClass().getDexCacheResolvedMethod(dexMethodIndex);
