...
public final class MessageFormat {
    ...
    private void format(int msgStart, double pluralNumber,
        ...
        for(int i=msgStart+1;; ++i) {
            ...
            if (noArg != null) {
                ...
                int subMsgStart=PluralFormat_findSubMessage(msgPattern, i, selector, number);
                double offset=msgPattern.getPluralOffset(i);
                formatComplexSubMessage(subMsgStart, number-offset, args, argsMap, dest);
            } else if(argType==ArgType.SELECT) {
                int subMsgStart=SelectFormat_findSubMessage(msgPattern, i, arg.toString());
                formatComplexSubMessage(subMsgStart, 0, args, argsMap, dest);
            } else {
     ...
     */
    private static int PluralFormat_findSubMessage(
     ...
     * @return the sub-message start part index.
     */
    private static int SelectFormat_findSubMessage(MessagePattern pattern, int partIndex, String keyword) {
      int count=pattern.countParts();
      int msgStart=0;
