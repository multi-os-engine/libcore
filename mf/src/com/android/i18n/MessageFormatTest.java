...
public class MessageFormatTest {
  ...
  public void testPerformance() throws Exception {
    ...
    for (int i = 0; i < 1024*100; ++i) {
      new MessageFormat("{0, plural, " +
                            "=0{No files on \"{1}\".}" +
                            "=1{One file on \"{1}\".}" +
                            "other{# files on \"{1}\".}}", Locale.US).format(123, "x");

      new MessageFormat("A \"{1}\" B {0} C", Locale.US).format(3, "x");

      new MessageFormat("{0, select, " +
                            "male{His thing.}" +
                            "female{Her thing.}" +
                            "other{Their thing.}}", Locale.US).format("female");
    }
