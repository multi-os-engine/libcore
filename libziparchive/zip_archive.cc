...
struct MemMapping {
    void* const addr;  // Start of data
    const size_t length;  // Length of data

    void* const baseAddr;  // page-aligned base address
    const size_t baseLength;  // length of mapping

    MemMapping(void* const addr, const size_t length,
               void* const baseAddr, const size_t baseLength)
        : addr(addr),
          length(length),
          baseAddr(baseAddr),
...
MemMapping *MapFileSegment(const int fd, const off_t start, const size_t length) {
    assert(pMap != NULL);
    ...
    void * const memPtr = mmap(NULL, actualLength, PROT_READ, MAP_FILE | MAP_SHARED,
                fd, actualStart);
    ...
    return new MemMapping(memPtr, actualLength, (uint8_t *) memPtr + adjust, length);
}

void ReleaseMappedSegment(MemMapping* pMap) {
...
int WriteFully(int fd, const void* buf, size_t count, const char* logMsg) {
   while (count != 0) {
...
int CopyFileToFile(int outFd, int inFd, size_t count) {
        ...
        ZipArchive* pArchive, off_t fileLength, size_t readAmount, uint8_t* scanBuf)
{
     ...
     * in pArchive.
     */
    const MemMapping* mapping = MapFileSegment(fd, dirOffset, dirSize);
    if (mapping == NULL) {
        ALOGW("Zip: cd map failed");
        return -1;
    } else {
      pArchive->mDirectoryMap = static_cast<void *>(mapping);
    }
 ...
 */
ZipEntry FindEntry(const ZipArchive* pArchive, const char* entryName)
{
 ...
 */
int GetEntryInfo(const ZipArchive* pArchive, ZipEntry entry,
...
int ExtractEntryToFile(const ZipArchive* pArchive,
    ...
    if (GetEntryInfo(pArchive, entry, &method, &uncompLen, &compLen,
