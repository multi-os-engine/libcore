...
class ScopedIcuLocale {
 public:
  ScopedIcuLocale(JNIEnv* env, jstring javaLocaleName) : mEnv(env) {
    ...
    // icu4c doesn't understand the Locale.toString "sr_BA_#Cyrl" backwards compatibility form.
    // Sadly, it silently just ignores anything after the #, so we need to manually check for that.
    if (strchr(localeName.c_str(), '#')) {
      jniThrowExceptionFmt(mEnv, "java/lang/IllegalArgumentException",
                           "Invalid locale passed to icu4c: %s", localeName.c_str());
      return;
    }
