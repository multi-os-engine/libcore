 ...
 public:
  ScopedIcuLocale(JNIEnv* env, jstring javaLocaleName) : mEnv(env) {
    ...
    if (javaLocaleName == NULL) {
      fprintf(stderr, "ScopedIcuLocale javaLocaleName == NULL\n");
    ...
    if (localeName.c_str() == NULL) {
      fprintf(stderr, "ScopedIcuLocale localeName.c_str() == NULL\n");
      return;
    }

    if (strchr(localeName.c_str(), '#')) {
      jniThrowExceptionFmt(mEnv, "java/lang/IllegalArgumentException", "Invalid locale passed to icu4c: %s", localeName.c_str());
      return;
    }

    mLocale = Locale::createFromName(localeName.c_str());

    fprintf(stderr, "ScopedIcuLocale %s %s %s\n", localeName.c_str(), mLocale.getName(), mLocale.isBogus() ? "bogus" : "good");
  }
